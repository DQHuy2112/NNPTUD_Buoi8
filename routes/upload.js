...
router.get('/import-users', function (req, res) {
    res.status(405).json({
        message: "Dùng POST method để upload file. Sử dụng Postman hoặc Thunder Client: POST /api/v1/upload/import-users, form-data với field 'file'"
    });
})
...
router.post('/import-users', uploadExcel.single('file'), async function (req, res, next) {
    ...
    res.send({ message: "File uploaded. Use POST /api/v1/users/import with this path.", filePath });
});
